...
4. 如果搜索不到，严禁编造数字。
5. 禁止推荐千亿以上大盘股，优先选择市值50-500亿的中小盘标的。
6. 每只个股的推荐必须基于近24小时内的具体事件驱动。
7. 不要写市值数据，市值将由系统后台自动补充。
...
【【任务五：事件驱动型个股推荐 — 基于最近24小时变化】

基于任务四推荐的3个板块，在**每个板块内各推荐3只**最直接受益于「近24小时内事件/新闻/数据变化」的A股个股。

⚠️ 核心原则（必须严格遵守）：
1. 选股依据必须是**近24小时内发生的事件、KOL推文、新闻、数据变化**，不能是泛泛的行业逻辑。
2. 优先选择市值50~500亿的中小盘标的，**禁止推荐千亿以上大盘股**，除非有极其强烈的事件驱动且找不到更小市值的替代标的。
3. 每只个股的“弹性逻辑”**必须明确绑定具体事件**，格式为：“该股因[具体事件]在[具体业务]上直接受益”。
4. 必须输出3个板块，每个板块3只个股，共9只。
5. 如果某个板块近24小时无明显事件驱动，则**减少该板块推荐数量**，从其他事件更丰富的板块中补足，最终仍凑够9只。

⚠️ 重要：**不要写任何市值数据**，包括"搜索失败"、"建议核实"等字样。市值将由系统后台自动填充，你只需输出股票代码和逻辑。

对每只个股提供：
- 股票名称及代码（必须含6位代码 + .SH/.SZ，如：兆易创新（603986.SH））
- **触发事件**：明确指出是哪条KOL推文/哪个新闻/哪个数据变化触发了这次推荐，引用原文关键信息，并标注时间
- **事件时效性**：该事件发生在近24小时内
- **弹性逻辑**：为什么这只股票对该事件最敏感（必须结合该公司的具体业务，不能泛泛说“受益于行业增长”）
- **核心驱动**：该公司最直接受益于该事件的具体业务点
- **主要风险**：1-2条

任务五输出格式模板 - 严格按照此格式输出：
▶ 板块一：[板块名称]
  板块核心逻辑（一句话回顾）：[基于任务四的结论，但必须强调近24小时的关键变化]

  推荐个股：
  1. [股票名称]（[代码]，[市场]）
     触发事件：[具体事件，引用KOL推文/新闻，标注时间]
     事件时效性：近24小时内
     弹性逻辑：[该股因[具体事件]在[具体业务]上直接受益]
     核心驱动：[该公司最直接受益的业务点]
     主要风险：[1-2条]
  ...
  3. [股票名称]（[代码]，[市场]）
     ...（同上）
▶ 板块二：[板块名称]
  ...（同上格式，3只个股，必须基于近24小时事件）
▶ 板块三：[板块名称]
  ...（同上格式，3只个股，必须基于近24小时事件）

⚠️ 再次强调：
- 禁止推荐千亿以上大盘股作为首选
- 每只个股的推荐理由必须引用具体事件/新闻/推文
- 不要写市值数据
